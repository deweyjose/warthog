...
export class Config {
  ...
  async loadStaticConfigSync() {
    const config = this.loadStaticConfigFileSync();
    const constantized = ObjectUtil.constantizeKeys(config);
  ...
  private loadStaticConfigFileSync(): Partial<StaticConfigFile> {
