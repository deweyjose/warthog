...
export class Server<C extends BaseContext> {
  ...
  constructor(
    ...
    if(this.appOptions.host) {
      throw new Error("`host` option has been removed, please set `WARTHOG_APP_HOST` environment variable instead")
    }
    if(this.appOptions.port) {
      throw new Error("`port` option has been removed, please set `WARTHOG_APP_PORT` environment variable instead")
    }
    if(this.appOptions.generatedFolder) {
      throw new Error("`generatedFolder` option has been removed, please set `WARTHOG_GENERATED_FOLDER` environment variable instead")
    }
  ...
  async establishDBConnection(): Promise<Connection> {
    if (!this.connection) {
      ...
      const connectionFn = this.appConfig.get('MOCK_DATABASE') ? mockDBConnection : createDBConnection;
